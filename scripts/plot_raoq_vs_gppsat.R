...
# ## Phenodiv ----
# m1 <- dat %>% drop_na(GPP_sat, sat_phen_std) %>% lm(formula = "GPP_sat ~ sat_phen_std")
# p_scatter4 <- dat %>%
#   ggplot(aes(x = sat_phen_std, y = GPP_sat)) +
#   geom_smooth(method = "lm", formula = 'y ~ x', na.rm = T, color = "gray25") +
#   geom_point(aes(fill = IGBP), color = point_border_color, shape = point_shape, size = point_size_small, na.rm = T) +
#   scale_fill_manual(values = CatCol_igbp, guide = guide_legend(title = "IGBP class")) +
#   labs(caption = glue::glue(
#     "y = {round(m1$coefficients[2], digits = 2)}x + {round(m1$coefficients[1], digits = 2)};   R^2 = {round(summary(m1)$r.squared, digits = 3) * 100}%;   n = {dat %>% drop_na(GPP_sat, sat_phen_std) %>% nrow()}")
#   ) +
#   xlab("Phenological diversity from S2") + ylab("") + # axis title
#   theme_bw() +
#   NULL
# 
## Bands ----
...
if (savedata) {
  ...
  ggsave(filename = glue::glue("results/scatterplots/figure1_GPP_sat-raoQ_{vers_out}.jpg"),
         plot = figure1, device = "jpeg",
